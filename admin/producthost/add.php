...
if (!empty($_POST)) {
  ...
  if (!empty($title)) {
    ...
    if ($id == '') {
      $sql = 'insert into productshost(title,thumbnail,price,discount,content,id_category, created_at, update_at) values("' . $title . '","' . $thumbnail . '","' . $price. '","' . $discount. '","' . $content . '","' . $id_category . '", "' . $created_at . '","' . $update_at . '" ) ';
    } else {
      $sql = 'update productshost set title = "' . $title . '", update_at="' . $update_at . '", thumbnail="' . $thumbnail . '", price="' . $price . '", discount="' . $discount. '", content="' . $content . '", id_category="' . $id_category . '" where id= ' . $id;
    }
    execute($sql);
    // Luu vào database
...
if (isset($_GET['id'])) {
  $id = (int) $_GET['id'];
  $sql = 'select * from productshost where id=' . $id;
  $producthost = executeSingleResult($sql);
  if ($producthost != null) {
    $title = $producthost['title'];
    $price = $producthost['price'];
    $discount = $producthost['discount'];
    $thumbnail = $producthost['thumbnail'];
    $content = $producthost['content'];
    $id_category = $producthost['id_category'];
  }
}
...
<body>
  <ul class="nav nav-tabs">
    ...
    <li class="nav-item">
      <a class="nav-link active" href="../index.php">Quản Lý Sản Phẩm Mới</a>
    </li>
  </ul>
  <div class="container">
