...
include_once('layouts/bia.php');
// $item_page= !empty($_GET['per_page'])? $_GET['per_page']:8;
// $current_page= !empty($_GET['page'])? $_GET['page']:1;

// $offset = ($current_page - 1)* $item_page;
// $productList = executeResult("select * from products order by id ASC LIMIT ".$item_page." OFFSET ".$offset);
...
if ($search) {
  // echo 1111;exit;
  $productList = executeResult("select * from products WHERE title LIKE '%" . $search . "%' order by id ASC LIMIT " . $item_page . " OFFSET " . $offset);
  $totalRecords = mysqli_query($con, "select * from products  WHERE title LIKE '%" . $search . "%' ");
} else {
  // echo 2222;exit;
  $productList = executeResult("select * from products order by id ASC LIMIT " . $item_page . " OFFSET " . $offset);
...
<div class="container ">
  ...
  <div class="row">
    ...
    foreach ($productList as $item) {
      ...
      <a href="detail.php?id=' . $item['id'] . '"><img src="' . $item['thumbnail'] . '" alt="" style="width: 100%; height:350px"></a>
